...
async function initializeSuperAdmin() {
    ...
    try {
        ...
        if (existingSuperAdmin) {
            ...
            const result = await dataSource.query('SELECT password FROM users WHERE id = $1', [existingSuperAdmin.id]);
        ...
        const result = await dataSource.query('SELECT password FROM users WHERE id = $1', [superAdmin.id]);
        console.log('   Password hash (first 30 chars):', result[0]?.password?.substring(0, 30));
...
//# sourceMappingURL=init-super-admin.js.map